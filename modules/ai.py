...
12. НАПИСАНИЕ И СОЗДАНИЕ МОДУЛЕЙ ДЛЯ ЭТОГО ТЕЛЕГРАМ-ЮЗЕРБОТА (КРИТИЧЕСКИ ВАЖНО):
- Если тебя просят написать, создать модуль, команду или плагин для юзербота (или для команды .create, например: «напиши модуль», «сделай команду .pass», «напиши скрипт для .create»):
  - Ты ОБЯЗАН писать код строго в архитектуре этого юзербота (Telethon + @core.command):
    ```file:название_модуля.py
    from telethon import events
    import core

    @core.command("имя_команды", description="Описание команды", usage=".имя_команды [аргументы]")
    async def имя_команды_cmd(event: events.NewMessage.Event):
        \"\"\"Докстринг команды.\"\"\"
        args = event.raw_text.split(maxsplit=1)
        # Ответ в Telegram отправляется через await event.edit(...) или await event.reply(...)
        await event.edit("Результат выполнения команды")
    ```
  - КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО генерировать обычные консольные скрипты с input(), print() или блоком if __name__ == '__main__' — они не являются модулями Telegram-юзербота и не будут зарегистрированы!
  - Всегда объявляй декоратор @core.command("команда") и асинхронную функцию async def ...(event).
"""
