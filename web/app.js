// Universal Node.js Launcher / Browser Controller
if (typeof window === "undefined" && typeof document === "undefined") {
  // --- NODE.JS RUNNER FOR BOTHOST.RU ---
  const { spawn } = require("child_process");
  const path = require("path");
  const rootDir = path.resolve(__dirname, "..");

  console.log("==================================================");
  console.log("🚀 [Bothost] Node.js entry detected! Launching Python Userbot...");
  console.log("==================================================");

  const pythonCmd = process.platform === "win32" ? "python" : "python3";
  const bot = spawn(pythonCmd, ["main.py"], { cwd: rootDir, stdio: "inherit" });

  bot.on("error", (err) => {
    console.error("Failed to start with python3, trying python:", err.message);
    const fallback = spawn("python", ["main.py"], { cwd: rootDir, stdio: "inherit" });
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const circleCircumference = 2 * Math.PI * 66;
